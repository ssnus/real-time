...
export class ColumnController {
  ...
  @Post()
  create(@Req() req: any, @Param('boardId') boardId: string, @Body() dto: CreateColumnDto) {
    const userId = req.user.sub;
    return this.columnService.create(userId, boardId, dto);
  }

  @Get()
  findAll(@Req() req: any, @Param('boardId') boardId: string) {
    const userId = req.user.sub;
    return this.columnService.findAll(userId, boardId);
  }

  @Put(':id')
  update(@Req() req: any, @Param('id') id: string, @Body() dto: UpdateColumnDto) {
    const userId = req.user.sub;
